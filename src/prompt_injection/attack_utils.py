...
class EscapeAttack(BasePromptInjectionBaseline):
    ...
    def __init__(self, injected_prompt: str = None, variant: str = 'deletion'):
        '''
        attack_type:
    ...
    def generate_attack_prompt(self, benign_question: str) -> str:
        ...
        elif self.variant == 'separation':  # separation
    ...
    def _get_completion_suffix(self) -> str:
        ...
        elif self.variant == 'separation':
            char = ('\n' if np.random.rand() < 0.5 else '\t') * np.random.choice(range(10))
            return char + self.injected_prompt.capitalize()
        else:
            raise ValueError(f"Unsupported attack type: {self.variant}")
...
class CompletionAttack(BasePromptInjectionBaseline):
    ...
    def __init__(self, injected_prompt: str, variant: str = 'real'):
        '''
        variant:
    ...
    def generate_attack_prompt(self, benign_question: str) -> str:
        return benign_question + self._get_completion_suffix()
...
def create_attack_instance(method_name: str, injected_prompt: str) -> BasePromptInjectionBaseline:
    """
    ...
    if method_name in ['none', 'no_attack']:
        ...
        return NaiveAttack(injected_prompt=injected_prompt)
    elif method_name in ['ignore', 'ignore_attack']:
        return IgnoreAttack(injected_prompt=injected_prompt)
    elif method_name in ['escape_deletion', 'escape_separation']:
        variant = 'deletion' if 'deletion' in method_name else 'separation'
        return EscapeAttack(variant=variant, injected_prompt=injected_prompt)
        ...
        return CompletionAttack(variant=variant, injected_prompt=injected_prompt)
        ...
        return StructureAttack(model_type=model_type, injected_prompt=injected_prompt)
    elif method_name in ['gcg', 'gcg_attack']:
        return GCGAttack(injected_prompt=injected_prompt)
    else:
